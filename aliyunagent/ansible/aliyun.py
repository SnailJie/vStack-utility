...
# create log
logger_dir = "/var/log/aliyun/"
create_log(logger_dir)
banner("Starting to deploy aliyun agent")
start_time = datetime.now()
# set default value
file_root = "files/aliyun"
...
parser = argparse.ArgumentParser(description='Deploy aliyun to host')
...
virtenv_path = "%s/virtualenv/aliyun/" % zstack_root
workplace = "%s/aliyun" % zstack_root
...
if distro == "RedHat" or distro == "CentOS":
    ...
    if zstack_repo != 'false':
        ...
        if distro_version >= 7:
            ...
            for pkg in [ 'iptables-services', 'collectd-virt']:
                yum_install_package(pkg, host_post_info)
        else:
            for pkg in [ 'qemu-img']:
                yum_install_package(pkg, host_post_info)
...
copy_arg = CopyArg()
copy_arg.src = "files/aliyun/zstack-aliyunagent"
...
if chroot_env == 'false':
    ...
    if distro == "RedHat" or distro == "CentOS":
        command = "service zstack-aliyunagent stop && service zstack-aliyunagent start && chkconfig zstack-kvmagent on"
    elif distro == "Debian" or distro == "Ubuntu":
    ...
    host_post_info.post_label_param = "zstack-aliyunagent"
    run_remote_command(command, host_post_info)
